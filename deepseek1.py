...
def load_biomedclip_model(model_name, device):
    """Load BiomedCLIP model with proper handling"""
    try:
        # Try to load from local cache first
        from open_clip import create_model_from_pretrained
        
        print("Attempting to load BiomedCLIP from pretrained...")
        model, preprocess = create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT-base-256')
        ...
        try:
            ...
            print(f"Failed to load standard CLIP: {e2}")
            raise
...
def main():
    ...
    # Load model
    try:
        clip_model, preprocess, tokenizer = load_biomedclip_model(args.model_name, device)

...
class MedTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.seg_flag <= 0:
            ...
            return x_img, torch.tensor(y_label, dtype=torch.float32), torch.zeros([1, self.resize, self.resize], dtype=torch.float32)
        ...
        return x_img, torch.tensor(y_label, dtype=torch.float32), mask
        ...
        for images, y_labels, masks in tqdm(test_loader, desc="Testing"):
            ...
            if seg_present:
                ...
                for i in range(batch_size):
                    # Use actual masks if available
                    mask_np = masks[i].squeeze().numpy()
                    gt_mask_list.append(mask_np)
                    # Create dummy segmentation scores (you can replace this with actual patch-based scoring)
                    dummy_seg_score = np.ones((args.img_size, args.img_size)) * sims[i].cpu().numpy()
                    segment_scores.append(dummy_seg_score)

    # convert to numpy arrays
    gt_arr = np.array(gt_list)
    image_scores = np.array(image_scores)
    
    # Normalize image scores to [0,1] using sigmoid
    image_scores_norm = 1 / (1 + np.exp(-image_scores))
    
    img_roc_auc_det = roc_auc_score(gt_arr, image_scores_norm)
    print(f'{args.obj} Image-level AUC : {round(img_roc_auc_det, 4)}')

    if seg_present and len(gt_mask_list) > 0 and len(segment_scores) > 0:
        ...
        valid_mask = ~(np.isnan(seg_scores) | np.isinf(seg_scores))
        gt_masks = gt_masks[valid_mask]
        seg_scores = seg_scores[valid_mask]
        
        if len(gt_masks) > 0 and len(seg_scores) > 0:
            # Normalize segmentation scores
            ...
            print(f'{args.obj} Pixel-level AUC: No valid segmentation data')
...
def main():
    ...
    # ---------------------------
    # Ensure checkpoints dir and download config/weights if needed
    # ---------------------------
    ckpt_dir = "checkpoints"
    os.makedirs(ckpt_dir, exist_ok=True)

    # Try to download BiomedCLIP model
    repo_id = "microsoft/BiomedCLIP-PubMedBERT-ViT-B-16"
    try:
        ...
        print(f"Warning: problem downloading assets from HF hub: {e}")
    ...
    try:
        ...
        print(f"Warning: Could not get tokenizer for {args.model_name}, using default")
        tokenizer = get_tokenizer("ViT-B-16")
    
    model_path = os.path.join(ckpt_dir, "open_clip_pytorch_model.bin")
    ...
    try:
        clip_model, _, preprocess = create_model_and_transforms(
            ...
            image_mean=None,
            image_std=None,
            **({} if preprocess_cfg is None else {f"image_{k}": v for k, v in preprocess_cfg.items()})
        )
    except Exception as e:
    ...
    # run test
    result = test(args, clip_model, preprocess, test_loader, text_feature, device)

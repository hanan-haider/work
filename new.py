...
def test(args, model, tokenizer, test_loader, text_features):
    ...
    for image, y, mask in tqdm(test_loader, desc="Testing"):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            # Extract patch tokens from vision transformer
            x = model.visual.conv1(image)
            x = x.reshape(x.shape[0], x.shape[1], -1)
            x = x.permute(0, 2, 1)
            
            # Add class token
            class_token = model.visual.class_embedding.to(x.dtype) + torch.zeros(
            ...
            for i, block in enumerate(model.visual.transformer.resblocks):
                ...
                if (i + 1) in args.features_list:
                    patch_tokens = x.permute(1, 0, 2)[:, 1:, :]  # Remove CLS token
                    patch_tokens_list.append(patch_tokens)
            
            # If no specific layers, use final output
            if len(patch_tokens_list) == 0:
                x = x.permute(1, 0, 2)
                patch_tokens_list = [x[:, 1:, :]]
            
            # Compute image-level score
            anomaly_score = 0
            for tokens in patch_tokens_list:
                tokens_norm = tokens / tokens.norm(dim=-1, keepdim=True)
                similarity = 100.0 * tokens_norm @ text_features.T
                anomaly_map = torch.softmax(similarity, dim=-1)[:, :, 1]
                anomaly_score += anomaly_map.mean()
            
            anomaly_score = anomaly_score / len(patch_tokens_list)
            image_scores.append(anomaly_score.cpu().item())
            
            # Compute pixel-level anomaly map
            anomaly_maps = []
            for tokens in patch_tokens_list:
                tokens_norm = tokens / tokens.norm(dim=-1, keepdim=True)
                similarity = 100.0 * tokens_norm @ text_features.T
                
                B, L, C = similarity.shape
                H = int(np.sqrt(L))
                
                similarity = similarity.permute(0, 2, 1).view(B, C, H, H)
            ...
            gt_mask_list.append(mask.squeeze())
            gt_list.append(y.cpu().item())
            segment_scores.append(final_score_map.squeeze())
    ...
    # Normalize scores
    if segment_scores.max() > segment_scores.min():
    ...
    # Pixel-level metrics
    if gt_list.sum() > 0 and CLASS_INDEX[args.obj] > 0:
        anomaly_indices = gt_list == 1
        gt_mask_anomaly = gt_mask_list[anomaly_indices]
        segment_scores_anomaly = segment_scores[anomaly_indices]
        
        if gt_mask_anomaly.sum() > 0:
            pixel_roc_auc = roc_auc_score(gt_mask_anomaly.flatten(), segment_scores_anomaly.flatten())
            print(f'{args.obj} Pixel AUC: {round(pixel_roc_auc, 4)}')
            
            try:
                pro_score = compute_pro_metric(gt_mask_anomaly, segment_scores_anomaly)
